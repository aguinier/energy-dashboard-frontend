...
Green as of 2026-08-12 (ABL-292): **44 client test files / 575 tests** and
**45 server test files / 651 tests**, all passing, clean typecheck on both.
Fewer tests passing than that means something broke.

ABL-292 moved a file across the boundary, so the two counts moved in opposite
directions and neither is a regression: it **deleted** the client's
`lib/opsStatusThresholds.ts`/`.test.ts` (-1 client file, -15 cases, 45/590 ->
44/575) and **added** `server/src/lib/opsStatusThresholds.test.ts` plus derived
-state cases in `services/combinedOpsStatusService.test.ts` and
`routes/opsStatus.test.ts` (+1 server file, +31 cases, 44/620 -> 45/651).

Two measurement notes worth having before you diagnose a "failure":

- **Measure on a clean tree.** The 45/590 figure this entry replaces was taken
  in the primary checkout while a concurrent run's untracked files were on
  disk, which inflated it by a file. ABL-292's numbers were measured in a
  dedicated worktree at the same commit with nothing untracked — `git status`
  before you trust a count.
- **The `storage.setItem is not a function` failures did not reproduce.** The
  previous entry recorded 570/590 passing with 20 failing in
  `dashboardStore.test.ts`/`windowLabel.test.ts`; on 2026-08-12 all 590 passed
  in a clean worktree and all 575 pass after this change. That quirk is
