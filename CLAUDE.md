...
`publicAppGraph.test.ts`'s "exactly three modules import `better-sqlite3`"
assertion is unaffected. `createPublicApp`'s module list is **unchanged by this
...
commands plus the widened compliance check in `usage/usageCli.test.ts` (+9). It
touches no client test, and the client figure below is carried forward rather
than re-measured.

The file count needed no run and is the check to prefer: `git ls-tree -r
origin/main --name-only | grep -c '^server/src/.*\.test\.ts$'` returns 106 at
`01e3160`, plus the one `scripts/` file the server suite also discovers
(`server/vitest.config.ts:11`) = 107, and this tree returns 110 + 1.

The paragraph below is the previous measurement and is kept for the deltas it
explains; per this section's own rule it is only true of the tree it names.
