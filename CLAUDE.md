...
`getDashboardOverview` is deliberately **not** window-bounded — `RangeSegment`
exposes `+24h`/`+7d`, and `getDateRangeForPreset` starts those windows at now,
so bounding it would blank the tile for all 34 countries the moment the user
looks forward. Verified: under `next7d`, DE still reads `40.76 GW` while its
three windowed siblings correctly read `—`.
...
`timeRange` — the legacy `'24h'|'7d'|'30d'|'90d'|'1y'` enum that used to persist
alongside `timePreset` and duplicate the same concept — **is gone.** This
section used to say the duplication was deliberate because the `/dashboard/*`
endpoints took `TimeRange` as a closed enum "with no `start`/`end`
passthrough"; that is no longer true. `/dashboard/overview` and
`/dashboard/map` now accept an explicit `start`/`end` window and prefer it over
the enum (`server/src/routes/dashboard.ts:33-59`), the client computes that
window from `timePreset`/`timeOffset` via `getDateRangeForPreset`
(`client/src/hooks/useDashboardData.ts:146-161`), and the v5 migration deletes
a persisted `timeRange` so it can't outlive the field
(`client/src/store/migrate.ts:62-73`). Nothing in `client/src` declares or reads
it any more. The enum collapse it caused is what produced the "+24h"
mislabeled-window bug — see `windowLabel.ts`.
