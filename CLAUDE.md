...
Green as of 2026-08-22, measured on ABL-529 branched from `main` at `01e3160`,
in a per-issue execution worktree with `node_modules` junctioned from the primary
checkout, under **v24.18.0**: **109 server test files / 2,159 tests**, all
passing, `tsc --noEmit` exit 0. ABL-529 is **+2 files / +27 cases**
...
That delta reconciles exactly against the figure below, which is what makes both
trustworthy rather than either: `git ls-tree -r 01e3160 --name-only | grep -c
'^server/src/.*\.test\.ts$'` returns **106**, plus the one `scripts/` file the
server suite also discovers (`server/vitest.config.ts:11`) = **107**, and
107 + 2 = 109. The *test* count still needed a run; the file count did not.
