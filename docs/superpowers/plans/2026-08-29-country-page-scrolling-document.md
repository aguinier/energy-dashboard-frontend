...
Repeat Task 6's `<Figure>` pattern for price, generation mix, wind and net position, reusing `PriceTab`, `GenerationTab`, `WindOnshoreTab` + `WindOffshoreTab`, and `NetPositionTab` as the plot slots. Per the spec: figures 2 and 4 additionally carry `<AbleResidualStrip>`; figure 5 passes `metrics={undefined}` so `<AccuracyBadge>` renders nothing, and its footnote states in prose that no forecast is published for net position.

**Figure 3's badge is coverage-gated — read the spec's "Coverage gating on generation figures" section before building it.** It reports `accuracy?.solar?.tso?.dayAhead`, labelled **solar component only**, but ONLY where `services/solarCoverage.ts` returns verdict `consistent` for the country. Where the verdict is `partial_subset` or `unknown`, suppress the badge and state in the footnote that the forecast and the metered actuals describe different populations. Without this gate the Netherlands renders a green check beside a WAPE of 1,727.81% — a number the repo itself documents as "arithmetically correct and still not an accuracy figure" (`tsoForecastService.ts:532`), because ENTSO-E forecasts the whole Dutch fleet while our actuals are a grid-metered subset summing 18.28x smaller (ABL-325). Do not implement this as a country list; the verdict is computed per country from data already held. Wind figures are NOT gated — no wind counterpart to `solarCoverage.ts` exists, so their badges render on the grounds that no divergence has been measured, which is a weaker claim than verified and is recorded as such in the spec.
