 ...
 * (Three internal services still read the frozen table; that is ABL-399, and
 * this endpoint does not wait for it because it never adopts the defect.)
 *
 ...
 *   adds its own.
 *
 * Deciding what `renewable` and `hydro_total` mean on the new table is **ABL-399**
 * (ABL-324's remainder), which is open and is not this endpoint's decision to
 * take by picking a column. Until it closes, a request for either gets a 400
 * naming the six types that *are* served — a refusal a client can act on —
 * rather than a plausible number computed against the wrong basis. Both remain
