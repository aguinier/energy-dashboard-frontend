...
import { normalizeTimestamp, toIsoUtc } from '../utils/timestamp.js';
...
export function getDashboardOverview(
  ...
  // printed 37.27 GW under the label "CURRENT LOAD"). It is not: `RangeSegment`
  // exposes `+24h` and `+7d`, and `getDateRangeForPreset` starts those windows
  // at now, so a windowed query returns nothing for *every* country the moment
  // the user looks forward - trading one country's wrong number for all 34
  // countries' missing one.
  //
